...
void Item::display(std::ostream& os, bool full /*= false*/) const {
  ...
  /*TODO check: why is displaying more spaces than expected?
  ...
  if(full) {
    os << " From " << std::left << std::setfill(' ') << std::setw(field_width) << source <<
          " To " << std::left << std::setw(field_width) << destination << std::endl <<
          std::setw(field_width + CODE_WIDTH + 3) << std::right << " : " << details;
  }
  os << std::endl;
}
